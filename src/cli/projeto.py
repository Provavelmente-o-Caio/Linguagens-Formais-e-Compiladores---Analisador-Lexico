...
def tela_projeto(analisador):
    while True:
        ...
        table.add_row("3", "Gerar AFDs (para todas as ERs)")
        table.add_row("4", "Minimizar AFDs")
        table.add_row("5", "Unir AFDs via ε")
        table.add_row("6", "Determinizar (autômato unido)")
        table.add_row("7", "Visualizar tabelas de transição")
        ...
        if op == "1":
            ...
            if getattr(analisador, "afds", None):
                ...
                console.print("[yellow]Primeiro gere os AFDs (opção 3).[/yellow]")
            input("ENTER para continuar...")
        elif op == "5":
            ...
            if not getattr(analisador, "afn_unido", None):
                console.print("[yellow]Primeiro una os autômatos (opção 5).[/yellow]")
            else:
                try:
            ...
            if getattr(analisador, "afd_unido", None):
                ...
                console.print(Panel("[bold cyan]AFD unido (determinizado)[/bold cyan]"))
